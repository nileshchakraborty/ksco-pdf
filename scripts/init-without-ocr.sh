...
if [ ! -z "$PUID" ] && [ "$PUID" != "$(id -u kscoPDFuser)" ]; then
    usermod -o -u "$PUID" kscoPDFuser || true
fi


if [ ! -z "$PGID" ] && [ "$PGID" != "$(getent group kscoPDFgroup | cut -d: -f3)" ]; then
    groupmod -o -g "$PGID" kscoPDFgroup || true
fi
umask "$UMASK" || true

if [[ "$INSTALL_BOOK_AND_ADVANCED_HTML_OPS" == "true" && "$FAT_DOCKER" != "true" ]]; then
  echo "issue with calibre in current version, feature currently disabled on ksco-PDF"
...
if chown -R kscoPDFuser:kscoPDFgroup $HOME /logs /scripts /usr/share/fonts/opentype/noto /configs /customFiles /pipeline /app.jar; then
	chmod -R 755 /logs /scripts /usr/share/fonts/opentype/noto /configs /customFiles /pipeline /app.jar || true
    # If chown succeeds, execute the command as kscoPDFuser
    exec su-exec kscoPDFuser "$@"
else
