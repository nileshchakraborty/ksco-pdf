# ksco-PDF Developer Guide

## 1. Introduction

ksco-PDF is a robust, locally hosted, web-based PDF manipulation tool. This guide focuses on Docker-based development and testing, which is the recommended approach for working with the full version of ksco-PDF.

## 2. Project Overview

ksco-PDF is built using:
...
1. Clone the repository:
   ...
   ```bash
   git clone https://github.com/ksco-Tools/ksco-PDF.git
   cd ksco-PDF
   ```
...
4. Lombok Setup
ksco-PDF uses Lombok to reduce boilerplate code. Some IDEs, like Eclipse, don't support Lombok out of the box. To set up Lombok in your development environment:
Visit the [Lombok website](https://projectlombok.org/setup/) for installation instructions specific to your IDE.

5. Add environment variable
For local testing, you should generally be testing the full 'Security' version of ksco-PDF. To do this, you must add the environment flag DOCKER_ENABLE_SECURITY=true to your system and/or IDE build/run step.
...
├── configs/               # Configuration files used by ksco at runtime (generated at runtime)
...
ksco-PDF offers several Docker versions:
...
ksco-PDF provides several example Docker Compose files in the `exampleYmlFiles` directory, such as:
...
services:
  ksco-pdf:
    container_name: ksco-PDF-Security
    image: docker.kscoPDF.com/kscotools/ksco-pdf:latest
    deploy:
      resources:
        limits:
          memory: 4G
    ...
    ports:
      - "8080:8080"
    volumes:
      - /ksco/latest/data:/usr/share/tessdata:rw
      - /ksco/latest/config:/configs:rw
      - /ksco/latest/logs:/logs:rw
    environment:
      ...
      UI_APPNAME: ksco-PDF
      UI_HOMEDESCRIPTION: Demo site for ksco-PDF Latest with Security
      UI_APPNAMENAVBAR: ksco-PDF Latest
...
ksco-PDF uses different Docker images for various configurations. The build process is controlled by environment variables and uses specific Dockerfile variants. Here's how to build the Docker images:
...
3. Build the Docker images:
   ...
   ```bash
   docker build --no-cache --pull --build-arg VERSION_TAG=alpha -t kscotools/ksco-pdf:latest -f ./Dockerfile .
   ```

   For the ultra-lite version:

   ```bash
   docker build --no-cache --pull --build-arg VERSION_TAG=alpha -t kscotools/ksco-pdf:latest-ultra-lite -f ./Dockerfile.ultra-lite .
   ```
   ...
   docker build --no-cache --pull --build-arg VERSION_TAG=alpha -t kscotools/ksco-pdf:latest-fat -f ./Dockerfile.fat .
   ```
...
ksco-PDF provides a `test.sh` script in the root directory. This script builds all versions of ksco-PDF, checks that each version works, and runs Cucumber tests. It's recommended to run this script before submitting a final pull request.
...
For quick iterations and development of Java backend, JavaScript, and UI components, you can run and test ksco-PDF locally without Docker. This approach allows you to work on and verify changes to:
...
8. See additional [contributing guidelines](https://github.com/ksco-Tools/ksco-PDF/blob/main/CONTRIBUTING.md).
...
API documentation is available at `/swagger-ui/index.html` when running the application. You can also view the latest API documentation [here](https://app.swaggerhub.com/apis-docs/ksco-Tools/ksco-PDF/).

## 9. Customization

ksco-PDF can be customized through environment variables or a `settings.yml` file. Key customization options include:
...
```bash
docker run -p 8080:8080 -e APP_NAME="My PDF Tool" ksco-pdf:full
```
...
For managing language translations that affect multiple files, ksco-PDF provides a helper script:
...
Thymeleaf is a server-side Java HTML template engine. It is used in ksco-PDF to render dynamic web pages. Thymeleaf integrates heavily with Spring Boot.

### Thymeleaf overview

In ksco-PDF, Thymeleaf is used to create HTML templates that are rendered on the server side. These templates are located in the `src/main/resources/templates` directory. Thymeleaf templates use a combination of HTML and special Thymeleaf attributes to dynamically generate content.
...
1. **Create a New Controller:**
   - Create a new Java class in the `src/main/java/ksco/software/SPDF/controller/api` directory.
   - Annotate the class with `@RestController` and `@RequestMapping` to define the API endpoint.
   - Ensure to add API documentation annotations like `@Tag(name = "General", description = "General APIs")` and `@Operation(summary = "Crops a PDF document", description = "This operation takes an input PDF file and crops it according to the given coordinates. Input:PDF Output:PDF Type:SISO")`.

   ```java
   package ksco.software.SPDF.controller.api;
...
2. **Define the Service Layer:** (Not required but often useful)
   - Create a new service class in the `src/main/java/ksco/software/SPDF/service` directory.
   - Implement the business logic for the new feature.

   ```java
   package ksco.software.SPDF.service;
...
- Autowire the service class in the controller and use it to handle the API request.
  ...
  import org.springframework.web.bind.annotation.RequestMapping;
  import org.springframework.web.bind.annotation.RestController;
  import ksco.software.SPDF.service.NewFeatureService;
  import io.swagger.v3.oas.annotations.Operation;
  import io.swagger.v3.oas.annotations.tags.Tag;
...
2. **Create a New Controller for the UI:**
   - Create a new Java class in the `src/main/java/ksco/software/SPDF/controller/ui` directory.
   - Annotate the class with `@Controller` and `@RequestMapping` to define the UI endpoint.

   ```java
   package ksco.software.SPDF.controller.ui;
...
## Adding New Translations to Existing Language Files in ksco-PDF

When adding a new feature or modifying existing ones in ksco-PDF, you'll need to add new translation entries to the existing language files. Here's a step-by-step guide:
