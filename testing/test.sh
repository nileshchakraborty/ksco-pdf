...
capture_file_list() {
    ...
    docker exec $container_name sh -c "find / -type f \
        ...
        -not -path '/config/*' \
        -not -path '/logs/*' \
        -not -path '*/home/kscoPDFuser/.config/libreoffice/*' \
        -not -path '*/tmp/PDFBox*' \
        -not -path '*/tmp/hsperfdata_kscoPDFuser/*' \
        -not -path '*/tmp/lu*' \
        -not -path '*/tmp/tmp*' \
...
main() {
    ...
    docker build --build-arg VERSION_TAG=alpha -t docker.kscoPDF.com/kscotools/ksco-pdf:latest-ultra-lite -f ./Dockerfile.ultra-lite .

    # Test each configuration
    run_tests "ksco-PDF-Ultra-Lite" "./exampleYmlFiles/docker-compose-latest-ultra-lite.yml"
    ...
    # run_tests "ksco-PDF" "./exampleYmlFiles/docker-compose-latest.yml"
    ...
    docker build --no-cache --pull --build-arg VERSION_TAG=alpha -t docker.kscoPDF.com/kscotools/ksco-pdf:latest-fat -f ./Dockerfile.fat .


    # Test each configuration with security
    # run_tests "ksco-PDF-Ultra-Lite-Security" "./exampleYmlFiles/docker-compose-latest-ultra-lite-security.yml"
    # docker-compose -f "./exampleYmlFiles/docker-compose-latest-ultra-lite-security.yml" down
    # run_tests "ksco-PDF-Security" "./exampleYmlFiles/docker-compose-latest-security.yml"
    # docker-compose -f "./exampleYmlFiles/docker-compose-latest-security.yml" down


    run_tests "ksco-PDF-Security-Fat" "./exampleYmlFiles/docker-compose-latest-fat-security.yml"
    ...
    run_tests "ksco-PDF-Security-Fat-with-login" "./exampleYmlFiles/test_cicd.yml"
    ...
    if [ $? -eq 0 ]; then
        ...
        CONTAINER_NAME="ksco-PDF-Security-Fat-with-login"
        ...
        if python -m behave; then
            ...
            if compare_file_lists "$BEFORE_FILE" "$AFTER_FILE" "$DIFF_FILE" "$CONTAINER_NAME"; then
                ...
                passed_tests+=("ksco-PDF-Regression")
            else
                echo "WARNING: Unexpected temporary files detected after behave tests!"
                failed_tests+=("ksco-PDF-Regression-Temp-Files")
            fi
            
            passed_tests+=("ksco-PDF-Regression")
        else
            failed_tests+=("ksco-PDF-Regression")
    ...
    run_tests "ksco-PDF-Fat-Disable-Endpoints" "./exampleYmlFiles/docker-compose-latest-fat-endpoints-disabled.yml"
