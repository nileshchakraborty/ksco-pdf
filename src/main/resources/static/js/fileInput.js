...
function setupFileInput(chooser) {
  ...
  $('#' + elementId).on('change', async function (e) {
    ...
    const uploadLimit = window.kscoPDF?.uploadLimit ?? 0;
    if (uploadLimit > 0) {
      const oversizedFiles = allFiles.filter(f => f.size > uploadLimit);
      if (oversizedFiles.length > 0) {
        const names = oversizedFiles.map(f => `"${f.name}"`).join(', ');
        if (names.length === 1) {
          alert(`${names} ${window.kscoPDF.uploadLimitExceededSingular} ${window.kscoPDF.uploadLimitReadable}.`);
        } else {
          alert(`${names} ${window.kscoPDF.uploadLimitExceededPlural} ${window.kscoPDF.uploadLimitReadable}.`);
        }
