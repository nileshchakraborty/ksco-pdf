...
async function getLatestReleaseVersion() {
  const url = "https://api.github.com/repos/ksco-Tools/ksco-PDF/releases/latest";
  try {
...
async function getCurrentVersionFromBypass() {
  const url = "https://raw.githubusercontent.com/ksco-Tools/ksco-PDF/master/build.gradle";
  try {
    const response = await fetch(url);
    if (response.status === 200) {
